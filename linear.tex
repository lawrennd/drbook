...
\newglossaryentry{prior_distribution}{name={prior distribution},
  description={The prior distribution (or density) encapsulates our
    beliefs about the values of a parameter, or variable, before we
...
\newglossaryentry{Jensens_inequality}{ name={Jensen's inequality},
  ...
  \newglossaryentry{sample_mean}{name={sample mean}, description={A
      sample mean is the value obtained when the data points are
      summed together and divided by the total number of data
      points. It is a sample based approximation of the true mean,
      which is the expected value of a data point}}
  \newglossaryentry{centering_matrix}{name={centering matrix},
    description={the centering matrix, denoted in this book by
      $\centeringMatrix$, is a matrix which, when premultiplying a
      design matrix returns a centered version of the data, where the
      center is defined by the sample mean of the data set}}
  ...
  \newglossaryentry{variational_free_energy}{name={variational free
      energy}, description={the variational free
      energy\index{variational free energy} ...}}
  \newglossaryentry{entropy}{name={entropy}, description={The
      entropy\index{entropy} of a distribution is given by the
      expectation of its negative log likelihood $E(p(x)) =
      -\expDist{\log p(x)}{p(x)}$}}
...
(which we refer to as latent variables)\index{latent variable} were drawn from a Gaussian
...
\begin{figure}
  ...
  \end{matlab}
  \begin{center}
    \inputdiagram{../../../dimred/tex/diagrams/linearMapping3DPlot}
  \end{center}
...
\refbox{box:inversion}) to rewrite this as
\[
(\dataStd^{-2}\eye -
...
The marginal likelihood we have derived is in the same form of that we
...
$\latentDim=\dataDim-1$, then the Gaussian becomes unconstrained. \fixme{why $\dataDim-1$ and not just $\dataDim$.}
...
properties of multivariate Gaussians\book{ (see
  \refbox{box:multivariateGaussian})}{}.

The dimensionality reduction model we specified works as follows: each
...
\begin{figure}
  \begin{center}
    \input{../../../gplvm/tex/diagrams/ppcaGraph.pdf_t}
  \end{center}
...
\begin{boxfloat}
  ...
  \boxfontsize The Kullback-Leibler divergence\index{Kullback-Leibler
    (KL) divergence} \cite{Kullback:info51}, also known as the
  relative entropy \fixme{AND WHAT ELSE} is an information theoretic
...
it as an approximation to the KL-divergence\index{Kullback-Leibler (KL)
...
\emph{precision} or \emph{information
  matrix}.\index{\gls{precision_matrix}}\index{information
  matrix|see{precision matrix}} This matrix is known to be positive
...
In other words, the maximum likelihood solution for the mean vector is
...
Up to constant terms the negative log likelihood\footnote{The
...
\]
\begin{boxfloat}
  \caption{The Trace of a Matrix\index{trace (of a
      matrix)}}\label{box:trace}
...
product directly (see \refint{int:gradientLogDeterminant}). 
\begin{intfloat}
...
\]
substituting into \refeq{eq:statPointMapping} we can write 
\detail{\begin{align*}
...
components. Expectation maximization\index{expectation maximization
  (EM) algorithm} \citep{Dempster:EM77} is an approach to maximum
...
\glspl{EM_algorithm}\index{expectation maximization (EM) algorithm} decompose
...
\refbox{box:kldivergence}) between the density $q(\latentMatrix)$ and
...
\text{d}\latentMatrix.
\] 
The KL divergence\index{Kullback-Leibler (KL) divergence} is positive
...
\refeq{eq:varLowerBound} and recover the true likelihood,
\begin{align*}
...
likelihood. The \gls{EM_algorithm}\index{expectation maximization (EM)
  algorithm} exploits this relationship to perform a two stage
...
The \gls{EM_algorithm} for probabilistic PCA starts by setting
...
\refeq{eq:posteriorLatentPoint}. We then maximize the variational
...
\begin{figure}
  ...
  %{
\end{matlab}
%\input{../../dimred/tex/diagrams/emUpdatePca}
\caption{The EM algorithm visualized. Plot shows the log likelihood
  ...
  divergence\index{Kullback-Leibler (KL) divergence} between the true
...
\section{Bayesian Principal Components}\index{Bayesian PCA}
